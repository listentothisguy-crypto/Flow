...
- (BOOL)addCharacter:(NSString *)character
{
    if (self.characters.count >= 3)
        return NO;

    if ([self.characters containsObject:character])
        return NO;
...
- (BOOL)addIntention:(NSString *)intention
{
    if (self.intentions.count >= 3)
        return NO;

    if ([self.intentions containsObject:intention])
        return NO;

    [self.intentions addObject:intention];
    return YES;
}

- (BOOL)addEmotion:(NSString *)emotion
{
    if (self.emotions.count >= 3)
        return NO;

    if ([self.emotions containsObject:emotion])
        return NO;
...
- (id)init
{
    ...
    if (self)
    {
        ...
        intentions = [[NSMutableArray alloc] init];
        emotions = [[NSMutableArray alloc] init];
        sketches = [[NSMutableArray alloc] init];
